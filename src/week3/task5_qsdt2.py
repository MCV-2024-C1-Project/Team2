...
def extract_number_from_filename_qst2_w3(filename):
    '''Function to extract the number of the image'''
    match = re.search(r'(\d{5})_cropped_contour(\d{1})_w3\.pkl', filename)
    if match:
        return int(match.group(1))


directory_test = "filtered_cropped_qst2_w3"
...
# Initialize a dictionary to group files by image number
grouped_files = {}

# Group files by their image number (e.g., 00011_cropped_contour1.jpg and 00011_cropped_contour2.jpg)
for file in files_sorted:
    if file.endswith('_w3.pkl') and file != 'gt_corresps.pkl':
        index = extract_number_from_filename_qst2_w3(file)
...
for image_number, file_list in grouped_files.items():
    ...
    # Process each file in the group (contour1, contour2, etc.)
    for file_compare_image in file_list:
        pkl_grey_path = os.path.join(directory_test, file_compare_image)

        # Load the query image's descriptor
        with open(pkl_grey_path, 'rb') as pkl_file:
            histograms_first = pickle.load(pkl_file)

        distances = []

        # Compare against the BBDD
        for filename in os.listdir(directory_bbdd):
            if filename.endswith('_w3.pkl') and filename != 'relationships.pkl':
                ...
                # Load the database image's descriptor
        ...
        # Sort distances and select the top 10 results
        distances.sort(key=lambda x: x[0])  # Sort by distance (lowest first)
        top_k_10_results = [index for _, index in distances[:10]]

        # Append the top 10 results for this contour to the current image's results
        image_results.append(top_k_10_results)

    # Add the results for the current image (which may have one or two contours) to the final list
    list_results_k_10.append(image_results)
...
output_path = '../../results/week3/QST2/method1/result.pkl'
